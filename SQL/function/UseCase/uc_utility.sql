...
CREATE FUNCTION uc_hierarchyId (id VARCHAR(20) ) RETURNS VARCHAR(20) DETERMINISTIC
BEGIN
    IF LOCATE('.',id) >0
        THEN
...
CREATE FUNCTION generateIdUC ( IdPadre VARCHAR(20)) RETURNS VARCHAR(20) DETERMINISTIC
BEGIN
...
CREATE FUNCTION uc_legalKinship(IdUC VARCHAR(20), newPadre VARCHAR(20)) RETURNS TINYINT DETERMINISTIC
BEGIN
